...
function create() {
    ...
    bg = new FlxSprite(-300, -70, Paths.image('stages/Creepy Cutie shinaniganz/bg'));
    bg.scrollFactor.set(0.9, 0.9);
    bg.scale.set(1.8, 1.6);
    bg.updateHitbox();
    
    if (!Options.lowMemoryMode) {
        tre = new FlxSprite(-400, -90, Paths.image('stages/Creepy Cutie shinaniganz/tre'));
        tre.scrollFactor.set(1.5, 1.5);
        tre.scale.set(1.6, 2.3);
        tre.updateHitbox();
        
        tre2 = new FlxSprite(900, -90, Paths.image('stages/Creepy Cutie shinaniganz/tre2'));
